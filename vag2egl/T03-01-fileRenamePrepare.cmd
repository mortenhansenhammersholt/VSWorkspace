...
echo building files
   dir %V2EPARM_REPOSLOC%\%V2EPARM_PROJECT%\EGLSource\imp\%mask%.egl /A:-D /B /O:N > %rdir%\%temp%\dir.txt

   for /F "eol=; tokens=1,2* delims==" %%i in (%rdir%\%temp%\dir.txt) do (
     echo if exist %V2EPARM_REPOSLOC%\%V2EPARM_PROJECT%\EGLSource\imp\%pref%-%%i del %V2EPARM_REPOSLOC%\%V2EPARM_PROJECT%\EGLSource\imp\%pref%-%%i  >> %rdir%\W03-O02-fileRename.cmd
     echo ren %V2EPARM_REPOSLOC%\%V2EPARM_PROJECT%\EGLSource\imp\%%i %pref%-%%i >> %rdir%\W03-O02-fileRename.cmd
     )
