...
:process_mapg
   ...
   for /F "eol=; tokens=1,2* delims==" %%i in (%temp%\dir.txt) do (
     echo java com.ibm.etools.egl.internal.vagenmigration.batch.VGMIG -importFile %rdir%\%V2EPARM_MODULEPFX%\mapg\%%i -eglFile %V2EPARM_REPOSLOC%\%V2EPARM_PROJECT%\EGLSource\imp\%%i.egl -data %V2EPARM_REPOSLOC% -package imp -overwrite �� %rdir%\W01-O01-WDzImport.cmd
     )
...
:process_esf
   ...
   for /F "eol=; tokens=1,2* delims==" %%i in (%temp%\dir.txt) do (
     echo java com.ibm.etools.egl.internal.vagenmigration.batch.VGMIG -importFile %rdir%\%V2EPARM_MODULEPFX%\%%i -eglFile %V2EPARM_REPOSLOC%\%V2EPARM_PROJECT%\EGLSource\imp\%%i.egl -data %V2EPARM_REPOSLOC% -package imp -overwrite �� %rdir%\W01-O01-WDzImport.cmd
     )
